...
class Seo {
    ...
    /**
     * <strong>Obter MetaTags:</strong> Execute este método informando os valores de navegação para que o mesmo obtenha
     * todas as metas como title, description, og, itemgroup, etc.
     * 
     * <strong>Deve ser usada com um ECHO dentro da tag HEAD!</strong>
    ...
    /**
     * <strong>Obter Dados:</strong> Este será automaticamente povoado com valores de uma tabela single para arquivos
    ...
    private function getSeo() {
        ...
        switch ($this->File):
            ...
            case 'artigo':
                ...
                $ReadSeo->ExeRead("ni_posts", "WHERE {$Check} post_name = :link", "link={$this->Link}");
                if (!$ReadSeo->getResult()):
                    ...
                    $Update = new Update();
                    $Update->ExeUpdate("ni_posts", $ArrUpdate, "WHERE post_id = :postid", "postid={$post_id}");
                endif;
                break;

            //SEO:: CATEGORIA
            case 'categoria':
                $ReadSeo->ExeRead("ni_categories", "WHERE category_name = :link", "link={$this->Link}");
                if (!$ReadSeo->getResult()):
                    $this->seoData = null;
                    $this->seoTags = null;
                else:
                    ...
                    $Update = new Update();
                    $Update->ExeUpdate("ni_categories", $ArrUpdate, "WHERE category_id = :catid", "catid={$category_id}");
                endif;
                break;

            //SEO:: PESQUISA
            case 'pesquisa':
                $ReadSeo->ExeRead("ni_posts", "WHERE post_status = 1 AND (post_title LIKE '%' :link '%' OR post_content LIKE '%' :link '%')", "link={$this->Link}");
                if (!$ReadSeo->getResult()):
                    $this->seoData = null;
                    $this->seoTags = null;
                else:
