...
class Email {
    ...
    /**
     * <strong>Send SMTP Email:</strong> Setar dados do e-mail em um array atribuitivo para povoar o método.
     * Feito isto apenas execute o metodo para ter toda a validação 
     * de envio do e-mail feita automaticamente.
     * 
     * <strong>DADOS ESPECÍFICOS:</strong> Para enviar o e-mail você deve montar um array atribuitivo com os
    ...
    private function sendMail() {
        if ($this->Mail->Send()):
            $this->Error = ['Obrigado por entrar em contato: Recebemos sua mensagem e estaremos respondendo em breve!', NI_ACCEPT];
            $this->Result = true;
        else:
            $this->Error = ["Erro ao enviar: Entre em contato com o admin diretamente via e-mail. ( {$this->Mail->ErrorInfo} )", NI_ERROR];
            $this->Result = false;
        endif;
    }
