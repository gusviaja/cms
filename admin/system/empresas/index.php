...
<div class="content list_content">
    ...
    <section class="list_emp">
        ...
        if ($empty):
            NIErro("Oppsss: Você tentou editar uma empresa que não existe no sistema!", NI_INFOR);
        endif;
        ...
        if ($action):
            ...
            switch ($action):
                case 'active':
                    ...
                    NIErro("O status da empresa foi atualizado para <b>ativo</b>. Empresa publicado!", NI_ACCEPT);
                    break;

                case 'inative':
                    $empUpdate->ExeStatus($empAction, '0');
                    NIErro("O status da empresa foi atualizado para <b>inativo</b>. Empresa agora é um rascunho!", NI_ALERT);
                    break;

                case 'delete':
                    $empUpdate->ExeDelete($empAction);
                    NIErro($empUpdate->getError()[0], $empUpdate->getError()[1]);
                    break;
        ...
        if ($readEmp->getResult()):
            ...
            $Pager->ReturnPage();
            NIErro("Desculpe, ainda não existem empresas cadastradas!", NI_INFOR);
        endif;
        ?>
