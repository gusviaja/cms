...
class AdminPost {
    ...
    const Entity = 'ni_posts';

    /**
     * <strong>Cadastrar o Post:</strong> Envelope os dados do post em um array atribuitivo e execute esse método
    ...
    public function ExeCreate(array $Data) {
        ...
        if (in_array('', $this->Data)):
            $this->Error = ["Erro ao cadastrar: Para criar um post, favor preencha todos os campos!", NI_ALERT];
            $this->Result = false;
        else:
            $this->setData();
            $this->setName();
            ...
            if (isset($uplaod) && $uplaod->getResult()):
                ...
                $_SESSION['errCapa'] = "<strong>ERRO AO ENVIAR CAPA:</strong> Tipo de arquivo inválido, envie imagens JPG ou PNG!";
                $this->Create();
            endif;
        endif;
    }
    ...
    /**
     * <strong>Atualizar Post:</strong> Envelope os dados em uma array atribuitivo e informe o id de um 
    ...
    public function ExeUpdate($PostId, array $Data) {
        ...
        if (in_array('', $this->Data)):
            $this->Error = ["Para atualizar este post, preencha todos os campos ( Capa não precisa ser enviada! )", NI_ALERT];
            $this->Result = false;
        else:
            $this->setData();
            $this->setName();
            ...
            if (isset($uploadCapa) && $uploadCapa->getResult()):
                ...
                if (!empty($uploadCapa) && $uploadCapa->getError()):
                    WSErro("<strong>ERRO AO ENVIAR CAPA:</strong> " . $uploadCapa->getError(), E_USER_WARNING);
                endif;
                $this->Update();
            endif;
        endif;
    ...
    /**
     * <strong>Deleta Post:</strong> Informe o ID do post a ser removido para que esse método realize uma checagem de
    ...
    public function ExeDelete($PostId) {
        ...
        if (!$ReadPost->getResult()):
            $this->Error = ["O post que você tentou deletar não existe no sistema!", NI_ERROR];
            $this->Result = false;
        else:
            ...
            $readGallery->ExeRead("ni_posts_gallery", "WHERE post_id = :id", "id={$this->Post}");
            ...
            $deleta = new Delete;
            $deleta->ExeDelete("ni_posts_gallery", "WHERE post_id = :gbpost", "gbpost={$this->Post}");
            $deleta->ExeDelete(self::Entity, "WHERE post_id = :postid", "postid={$this->Post}");

            $this->Error = ["O post <strong>{$PostDelete['post_title']}</strong> foi removido com sucesso do sistema!", NI_ACCEPT];
            $this->Result = true;
    ...
    /**
     * <strong>Ativa/Inativa Post:</strong> Informe o ID do post e o status e um status sendo 1 para ativo e 0 para
    ...
    /**
     * <strong>Enviar Galeria:</strong> Envelope um $_FILES de um input multiple e envie junto a um postID para executar
    ...
    public function gbSend(array $Images, $PostId) {
        ...
        if (!$ImageName->getResult()):
            $this->Error = ["Erro ao enviar galeria. O índice {$this->Post} não foi encontrado no banco!", NI_ERROR];
            $this->Result = false;
        else:
            ...
            foreach ($gbFiles as $gbUpload):
                ...
                if ($gbSend->getResult()):
                    ...
                    $insertGb = new Create;
                    $insertGb->ExeCreate("ni_posts_gallery", $gbCreate);
                    $u++;
                endif;
            ...
            if ($u > 1):
                $this->Error = ["Galeria Atualizada: Foram enviadas {$u} imagens para galeria deste post!", NI_ACCEPT];
                $this->Result = true;
            endif;
        endif;
    }
    ...
    /**
     * <strong>Deletar Imagem da galeria:</strong> Informe apenas o id da imagem na galeria para que esse método leia e remova
    ...
    public function gbRemove($GbImageId) {
        ...
        $readGb = new Read;
        $readGb->ExeRead("ni_posts_gallery", "WHERE gallery_id = :gb", "gb={$this->Post}");
        if ($readGb->getResult()):
            ...
            $Deleta = new Delete;
            $Deleta->ExeDelete("ni_posts_gallery", "WHERE gallery_id = :id", "id={$this->Post}");
            if ($Deleta->getResult()):
                $this->Error = ["A imagem foi removida com sucesso da galeria!", NI_ACCEPT];
                $this->Result = true;
            endif;
    ...
    /**
     * <strong>Verificar Cadastro:</strong> Retorna ID do registro se o cadastro for efetuado ou FALSE se não.
    ...
    /**
     * <strong>Obter Erro:</strong> Retorna um array associativo com uma mensagem e o tipo de erro.
    ...
    private function getCatParent() {
        $rCat = new Read;
        $rCat->ExeRead("ni_categories", "WHERE category_id = :id", "id={$this->Data['post_category']}");
        if ($rCat->getResult()):
    ...
    private function Create() {
        ...
        if ($cadastra->getResult()):
            $this->Error = ["O post {$this->Data['post_title']} foi cadastrado com sucesso no sistema!", NI_ACCEPT];
    ...
    private function Update() {
        ...
        $Update->ExeUpdate(self::Entity, $this->Data, "WHERE post_id = :id", "id={$this->Post}");
        if ($Update->getResult()):
            $this->Error = ["O post <strong>{$this->Data['post_title']}</strong> foi atualizado com sucesso no sistema!", NI_ACCEPT];
            $this->Result = true;
        endif;
    }
